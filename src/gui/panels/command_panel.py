...
#          Supports operator and CICD modes.
#          Operator mode hides raw SCPI and check buttons.
#          CICD mode shows them.
...
class CommandPanel(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        self.title_row = ttk.Frame(self)
        self.title_row.pack(fill="x", pady=(0, 5))
        ...
        self.checks_right = ttk.Frame(self.title_row)
        self.checks_right.pack(side="right")
        ...
        self.command_entry = ttk.Entry(self.entry_row, textvariable=self.command_var, width=30)
    ...
    def set_mode(self, mode):
        """Show or hide advanced controls based on mode."""
        self.mode = mode

        if mode == "cicd":
            if not self.checks_right.winfo_manager():
                self.checks_right.pack(side="right")
            if not self.selected_label.winfo_manager():
                self.selected_label.pack(anchor="w", pady=(0, 5))
            if not self.entry_row.winfo_manager():
                self.entry_row.pack(fill="x", pady=(0, 5))
        else:
            if self.checks_right.winfo_manager():
                self.checks_right.pack_forget()
            if self.selected_label.winfo_manager():
                self.selected_label.pack_forget()
            if self.entry_row.winfo_manager():
